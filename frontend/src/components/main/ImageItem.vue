<template>
    <div v-if="item.files.length > 1" class="relative">
        <transition name="slide">
            <img v-if="currentImage" class="w-full" :src="this.backendURL + currentImage" alt="Slider Image" />
        </transition>
        <div class="absolute left-1/2 transform -translate-x-1/2 bottom-2 flex space-x-2">
            <button v-for="(image, i) in item.files" :key="i" class="w-6 h-2 bg-white" @click="changeImageTo(i)"
...
export default {
    ...
    methods: {
        changeImage() {
            this.imageIndex++;
            this.currentImage = this.item.files[this.imageIndex % this.item.files.length].path
        },
        changeImageTo(i) {
