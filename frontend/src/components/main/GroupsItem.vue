    ...
    <div v-if="currentGroup" @click.self="currentGroup = undefined"
        ...
        <div class="w-1/2 h-full bg-white">
            <div class="bg-primary px-3 py-5 flex justify-between items-end">
                <h2 class="text-white text-heading-3 text-6xl">{{ currentGroup.name }}</h2>
                <div class="relative rounded-full aspect-square w-[100px] flex justify-center items-center"
                    :style="`background-color: ${currentGroup.color}`">
                    <img :src="this.backendURL + currentGroup.file?.path" class="w-[60%] h-[60%]">
                </div>

            </div>
            <div class="p-8">
                <div class="border border-primary w-full p-3">
                    <p class="main-text text-lg"><span class="font-semibold">Alter:</span> {{ currentGroup.section.fromAge
                    }} - {{ currentGroup.section.toAge }} Jahre</p>
                    <p class="main-text text-lg"><span class="font-semibold">Geschlecht:</span> {{
                        getGenderName(currentGroup.gender) }}</p>
