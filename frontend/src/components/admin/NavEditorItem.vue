<template>
  ...
  >
    ...
    <div
      ...
      :class="{ 'border-gray-500 ring-2 ring-gray-200': dropPosition === 'inside' }"
    >
        <div 
        ...
        >
          <div class="flex items-center gap-3 overflow-hidden">
            ...
            <div class="flex flex-col truncate">
              <span class="font-medium text-gray-900 text-sm truncate">{{ localItem.title }}</span>
              <span class="text-xs text-gray-500 truncate">{{ itemTypeLabel }}</span>
            </div>
          </div>
        ...
        <div v-if="showEdit" class="p-4 border-t border-gray-100 bg-gray-50 rounded-b-md space-y-3 cursor-default">
            <TextInput
    ...
    ></div>
    <div v-if="localItem.children && localItem.children.length > 0" class="ml-6 pl-3 border-l-2 border-gray-100 space-y-2 mt-2 mb-2">
       <NavEditorItem
...
export default {
  ...
  computed: {
    itemTypeLabel() {
        ...
        if (this.localItem.type === 'customItem') return this.$t('dashboard.customLinks');
        if (this.localItem.page_id || this.localItem.pageId) return this.$t('dashboard.page');
    ...
    canEdit() {
        ...
        return !this.localItem.page_id && !this.localItem.pageId && !this.localItem.special;
    }
  },
  methods: {
    toggleEdit() {
    ...
    handleDragStart(e) {
      ...
      this.$emit('menu-item-drag-start', { item: this.localItem, element: this.$el });
    },
    handleDragEnd() {
