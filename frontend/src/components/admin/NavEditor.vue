<template>
  <Card class="min-h-[200px] flex flex-col">
    <div
      ...
      @drop="handleRootDrop"
    >
      <div v-if="localItems.length === 0" class="absolute inset-0 flex items-center justify-center text-gray-400 border-2 border-dashed border-gray-300 rounded-lg m-4">
      ...
      <div v-if="rootDropActive" class="h-2 bg-gray-500 rounded mt-2 transition-all"></div>
    </div>
  </Card>
</template>
...
export default {
  ...
  methods: {
    ...
    handleItemDrop(e) {
        ...
        this.handleDropLogic(e.draggedItem, e.source, e.targetId, e.position, e.targetLevel);
    },
    ...
    handleDropLogic(draggedItem, source, targetId, position, targetLevel = 0) {
        ...
        if (newLevel + draggedDepth > MAX_DEPTH) {
            alert(this.$t('dashboard.maxDepthExceeded') || "Max menu depth exceeded (2 levels only).");
            return;
        }

        let itemToInsert = draggedItem;
        if (source === 'available') {
            itemToInsert = { ...draggedItem, id: null, tempId: 'temp_' + Date.now() + Math.random(), children: [] };
        } else {
            
             const found = this.findAndRemove(this.localItems, itemToInsert.id || itemToInsert.tempId);
        ...
        if (targetId === null && position === 'end') {
            ...
            const { parent, index, node } = this.findNodeAndParent(this.localItems, targetId);
    ...
    findAndRemove(list, id) {
        for (let i = 0; i < list.length; i++) {
            if ((list[i].id && list[i].id === id) || (list[i].tempId && list[i].tempId === id)) {
    ...
    findNodeAndParent(list, id) {
        for (let i = 0; i < list.length; i++) {
            ...
            if ((item.id && item.id === id) || (item.tempId && item.tempId === id)) {
    ...
    updateItem(updatedItem) {
        ...
        const { node, parent, index } = this.findNodeAndParent(this.localItems, id);
        if (node && parent) {
