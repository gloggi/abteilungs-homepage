  ...
  <div>
    ...
    <div class="flex flex-col-reverse md:flex-row gap-6">
      ...
      <div class="w-full md:w-2/3 flex flex-col gap-8">
        <NavEditor :items="menuItems" @update:items="updateItems" />

        <div class="border-t pt-8">
          <h3 class="text-lg font-bold text-gray-800 mb-4">
            {{ $t("dashboard.footerLinks") }}
          </h3>
          <NavEditor :items="footerLinks" @update:items="updateFooterLinks" />
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    async saveAll() {
      ...
      try {
        ...
        const currentMenuIds = currentMenuRes.data.data
          .map((i) => flatten(i))
          .flat();
        ...
        function flatten(i) {
          return [
            i.id,
            ...(i.children ? i.children.map((c) => flatten(c)).flat() : []),
          ];
        }
        ...
        const menuToDelete = currentMenuIds.filter(
          (id) => !frontendMenuIds.has(id),
        );
        await Promise.all(
          menuToDelete.map((id) => this.callApi("delete", `menuitems/${id}`)),
        );
        ...
        const frontendFooterIds = new Set(
          this.footerLinks.map((i) => i.id).filter((id) => id),
        );
        const footerToDelete = currentFooterIds.filter(
          (id) => !frontendFooterIds.has(id),
        );

        await Promise.all(
          footerToDelete.map((id) =>
            this.callApi("delete", `footerlinks/${id}`),
          ),
        );
