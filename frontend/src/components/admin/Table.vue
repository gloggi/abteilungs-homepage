<template>
  <div :key="`table-${tableKey}`" class="bg-gray-50 w-full flex justify-between items-center p-3 rounded-t-md">
    <div class="w-10"><input type="checkbox" ref="masterbox" v-model="topCheckboxValue" @change="topCheckboxHandler"
        class="rounded focus:ring-0 focus:shadow-none ring-offset-0 text-gray-900"></div>
    <div v-for="(key, i) in getTitles" :key="`text-${i}`" class="text" :style="`width: ${columnWidth}%;`">
      {{ key }}
    </div>
  </div>
  <hr />
  ...
  <div v-for="(item, j) in content" :key="`row-${j}`"
    class="bg-white w-full p-3 border-b flex justify-between items-center">
    <div class="w-10"><input type="checkbox" @change="(e) => changeBox(e, item['id'])"
        v-model="checkBoxValues[item['id']]" class="rounded focus:ring-0 focus:shadow-none ring-offset-0 text-gray-900">
    </div>
    <div v-for="(key, i) in keys" :key="`text-${j}-${i}`" class="text" :style="`width: ${columnWidth}%;`">
      <template v-if="actions[key] && actions[key].actionName == 'link'">
        <router-link class="text-sm text-blue-600 hover:text-blue-900"
          :to="`${entity}/${item[actions[key].actionArgument]}`">{{ item[key] || 'No name provided' }}</router-link>
      </template>
  ...
  <PaginationNav :pageNumber="page" :itemsPerPage="itemsPerPage" :totalItems="totalItems"
