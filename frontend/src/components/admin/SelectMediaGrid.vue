<template>
    <div v-if="content" class="grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-4 p-3" :key="listKey">
        <div v-for="file in content" :key="file.id"
            :class="`bg-gray-200 rounded-lg h-w-full ${isItemIncluded(file) ? 'ring-4 ring-offset-2 ring-blue-400' : ''}`"
...
export default {
    ...
    methods: {
        ...
        async getMedia() {
            try {
                const response = await this.callApi("get", "/files", {}, { params: { extensions: this.extensions.join(",") } });
